...
{
    public class LinguagensStaticServices : ILinguagensService
    {
        public object ListaLinguagens { get; private set; }

        List<Linguagens> getLinguagens()
        {

            List<Linguagens> listaLinguagens = new List<Linguagens>();
            ListaLinguagens.Add( new Linguagens { Id = 1, Nome = "C#", Aplicacao = "C# (pronuncia-se 'See Sharp') é uma linguagem de programação moderna, orientada a objeto e fortemente digitada. O C# permite que os desenvolvedores criem muitos tipos de aplicativos seguros e robustos que são executados no ecossistema do .NET. O C# tem suas raízes na família de linguagens C e os programadores em C, C++, Java e JavaScript a reconhecerão imediatamente. Este tour fornece uma visão geral dos principais componentes da linguagem no C# 8 e anterior.", Lancamento = new DateTime(2000), Desenvolvedor = "Anders Hejlsberg", Logo = "https://ivaldo.dev/wp-content/uploads/2019/03/27_c-sharp-logo-filled.png" });
    ...
    public List<Linguagens> getAll(string busca = null, bool ord = false)
    {
        ...
        if (ord)
        {
            ...
            lista = lista.OrderBy(lin => lin.Nome).ToList();
            return lista;
        }
        return getLinguagens();
    }

    public bool create(Linguagens l)
    {

        try
        {
            List<Linguagens> l = getLinguagens();
            l.Id = l.Count() + 1;
            l.Add(l);
            return true;
        }
    ...
    {
        return getLinguagens().FirstOrDefault(lin => lin.Id == id);
    }


    public bool update(Linguagens lin)
    {
        return false;
    }
